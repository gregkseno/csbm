...
class СSBMTrainer:
    ...
    def markovian_projection(
        ...
        for batch in trange:
            ...
            with self.accelerator.accumulate():
                ...
                loss = 0
                pred_x_start_logits = self.models[fb](x_t, t, true_x_end) ######!!!!!! add to model true_x_end

                # KL-loss calculation
                true_q_posterior_logits = self.prior.posterior_logits(true_x_start, x_t, t,
                                                                      logits=False)  # Version of DDPM
            ...
            if self.step % self.eval_freq == 0:
                self.accelerator.print(
                    f'{fb.capitalize()} D-IMF iteration: {self.iteration}: kl_loss: {info["kl_loss"]}, ce_loss: {info["ce_loss"]}')
    ...
    def viz(
        ...
        with self.emas[fb].average_parameters():
            ...
            #Гипотеза: здесь добавление условия фактически тоже самое что и его отсутствие
            ...
            if self.codec is not None:
                trajectories = self.codec.decode_to_image(
                    trajectories.reshape(-1, *traj_start.shape[1:]))  # type: ignore
                trajectories = trajectories.reshape(-1, self.num_trajectories * self.num_translations,
                                                    *pred_x_start.shape[1:])  # type: ignore
