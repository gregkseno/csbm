...
class DiffusionModel(nn.Module):
    def __init__(
        ...
        net = []
        ch_prev = input_dim + timestep_dim + 2 ######!!!!!!add +2
    ...
    def forward(self, x_t: torch.Tensor, t: torch.Tensor, x_st: torch.Tensor) -> torch.Tensor: ######!!!!!!add x_st
        x_start_logits = self.net(torch.cat([x_t.float(), self.timestep_embedding(t), x_st.float()], dim=1)) ######!!!!!!add x_st
...
class D3PM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, x_st: torch.Tensor) -> torch.Tensor: ######!!!!!!add x_st
        x_one_hot = torch.nn.functional.one_hot(x, self.num_categories)
        mean = (self.num_categories - 1) / 2
        x = x / mean - 1
        return self.model(x, t, x_st) + x_one_hot ######!!!!!!add x_st

    def markov_sample(self, x: torch.Tensor, t: torch.Tensor, prior: Prior, x_st: torch.Tensor = None): ######!!!!!!add x_st
        ...
        pred_x_start_logits = self(x, t, x_st) ######!!!!!!add x_st
